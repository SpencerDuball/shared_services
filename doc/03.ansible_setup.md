...
In my homelab, servers are lumped into two main categories: control or worker. This is the breakdown that Kubernetes provides and most tasks will be grouped and run from this perspective, however there are plenty of tasks we need to do for all servers or just a grouping of similar servers (maybe based upon ISA + OS). For example, all Turing PI RK1 boards are grouped together too as we may need to run different tasks since the hardware is different from x86 machines.

Ansible has the concept of playbooks and roles where we can group related tasks together. This section will document at a high level the tasks we want to perform and what each playbook and role is for, the specifics of all of these tasks are documented in the Playbook itself found in the [`ansible/playbooks/`](/ansible/playbooks/) folder.

## Variables & Secrets

Before we get into the tasks/roles/playbooks for our servers, we will briefly mention about variables and secrets in Ansible. There are variables we may want to configure for commonly used reference, or secrets to reference in our tasks such as API keys. In the project we have the [`group_vars/`](/ansible/group_vars/) folder, this folder can have subfolders named the same as the groups in our `inventory.yaml` file, and then within those folders are the `vars.yaml` and `secret.yaml`:

```bash
├── ansible.cfg
├── essential.yaml
├── group_vars
│   └── all             # <-- Applies to the "all" group of hosts
│       ├── secret.yaml # <-- The encrypted secrets file
│       └── vars.yaml   # <-- The unencrypted vars file
└── inventory.yaml
```

To work with the `secret.yaml` file, we will use the common commands. See the full list of commands with `ansible-vault --help`:

```bash
ansible-vault create secret.yaml                # create a secret file
ansible-vault edit secret.yaml                  # edit/update the secret file (will ask for password)
ansible-playbook playbook.yaml --ask-vault-pass # run a playbook and tell ansible to ask us for the vault password
```
